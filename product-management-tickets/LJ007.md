## 🎟️ Product Ticket: LJ-007 — Ranking Logic (Bronze / Silver / Gold)
...
> *Effort → Score → Rank → Motivation*

⚠️ This is **logic-only**. No UI, no leaderboards, no streaks.
...
- `DiagnosticTest.rank` field + migration shipped with deterministic Bronze/Silver/Gold thresholds (40/50/51/70/71/100) and stored per test.
- `assessments/ranking.py` service wired into the existing completion signal ensures rank assignment occurs exactly once alongside Treasure Chest unlocks, with idempotent behavior and no cross-impact on AI generation.
- Parents now see rank in diagnostic responses/admin while access control remains scoped to the owning parent.
- Automated suite (`python manage.py test assessments rewards`) covers all boundary cases, below-threshold scenario, and post-completion idempotency; tests rerun post-merge remain green.
- Runbook + ticket record the ranking thresholds, determinism expectations, and migrate prerequisite. LJ-007 closes the effort→score→rank feedback loop for MVP.
...
- Risks: deterministic mapping depends on stored `score_percentage`; edits to `correct_answers` post-completion no longer affect rank (by design) so PM should ensure user flows don’t expect re-ranking. No further blockers observed.
