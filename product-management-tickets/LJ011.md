## 🎟️ Product Ticket: LJ-011 — Hardening Pass (Low-Risk Robustness + UX)
...
2. **Documentation robustness**
   ...
   - Add “common deploy failures” troubleshooting section (CSRF host mismatch, missing `SECRET_KEY`, AI misconfig).
...
**Update 2025-12-17 — Full Stack Engineer (UX Fix)**
- Adjusted the homepage behavior for authenticated users: `GET /` now redirects to `/children/` so successful login does not drop users back onto the public landing page.
- Added regression coverage to ensure authenticated homepage redirect remains in place.
