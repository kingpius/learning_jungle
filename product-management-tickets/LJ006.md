## 🎟️ Product Ticket: LJ-006 — AI Diagnostic Question Generation (Maths Only)
...
Introduce **AI-powered generation of Maths diagnostic questions** aligned to the **UK National Curriculum**, tailored to a child’s **age and year group**.
...
⚠️ This ticket focuses on **generation only**, not learning paths, rankings, or UI polish.
...
* Child year group
* Subject = Maths
* Number of questions (configurable, default 10–15)
...
* Prompts must explicitly instruct the AI to:
  ...
  * Avoid content above the child’s year level
...
P0 — Core MVP intelligence layer

---

### 🔄 Updates & Coordination Log

**Update 2025-12-17 — PM**
...
**Update 2025-12-17 — Full Stack Engineer**
- Repo now contains `feature/LJ-006-ai-diagnostics-maths` (appears pre-created server side), but local Git still cannot write lock files (`.git/index.lock` permission denied), so I can’t check out the branch or stage commits.
...
**Update 2025-12-17 — Test Engineer**
...
- Outstanding risks: requirement “same inputs → same questions” currently depends on upstream provider honoring the provided seed; there is no caching by `(age, year)` yet, so we should document/monitor in staging once live responses are enabled.
...
**Update 2025-12-17 — Full Stack Engineer**
- Branch pushed: `feature/LJ-006-ai-diagnostics-maths` and PR opened: <https://github.com/kingpius/learning_jungle/compare/feature/LJ-006-ai-diagnostics-maths?expand=1>. Description includes summary, acceptance mapping, determinism caveat (“seed enforced but true repeatability depends on provider; caching planned post-MVP”), and explicit reminder to run `python manage.py migrate` before tests. Test output attached: `python manage.py migrate && python manage.py test assessments rewards` (10 tests, green).
...
- All 10 tests still green; no pending migrations. PR updated with comment confirming rerun so reviewers know merge didn’t introduce regressions.

**Final Update 2025-12-17 — PM**
