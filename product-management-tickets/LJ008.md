## 🎟️ Product Ticket: LJ-008 — Child Diagnostic Test UI (10–15 Minute Learning Flow)
...
* Questions displayed with four answer options (A–D)
...
* Must reuse existing models and logic (LJ-003 → LJ-007)
...
* LJ-004 — Child Profile Model & CRUD ✅
* LJ-005 — Diagnostic Test Model & Completion Event ✅
* LJ-006 — AI Diagnostic Question Generation (Maths Only) ✅
* LJ-007 — Ranking Logic (Bronze / Silver / Gold) ✅
...
P0 — MVP usability completion

---

### 🔄 Updates & Coordination Log

**Update 2025-12-18 — PM**
- We’re building on LJ-006/007 foundations. The UI must orchestrate existing backend services (question retrieval, answer submission, ranking, treasure unlock). No new business logic client-side.
...
- **Test Engineer:** prepare functional flow tests (start → answer selection → submit → results) plus edge cases (incomplete answers, completed test revisit read-only). Ensure they run headless via Django test client; no manual QA dependencies.
...
**Update 2025-12-18 — Full Stack Engineer**
- Proposed flow (all server-rendered Django templates under new `diagnostics` app section):
  1. **Test Overview (`GET /diagnostics/<child_id>/start/`)** – parent logs in, selects child, view shows latest pending Maths diagnostic (or CTA to create). POST to same route creates `DiagnosticTest` via existing service (reusing LJ-006 question generation) and redirects to question flow.
  2. **Question Flow (`GET/POST /diagnostics/tests/<uuid:test_id>/questions/`)** – displays questions one-per-page (with simple pagination) or stacked list; stores child selections in a new `DiagnosticResponse` model keyed to (`test`, `question`). View enforces parent ownership via `child__parent=request.user`. POST validates that each question has exactly one answer before allowing “Submit Test”.
  3. **Submission (`POST /diagnostics/tests/<uuid:test_id>/submit/`)** – service computes `correct_answers` by comparing `DiagnosticResponse.selected_option` to `DiagnosticQuestion.correct_option`, updates `DiagnosticTest.correct_answers`, and calls existing `DiagnosticTest.complete()` so ranking + treasure chest unlock fire. Endpoint rejects re-submission if `is_completed=True`.
  4. **Results (`GET /diagnostics/tests/<uuid:test_id>/results/`)** – read-only summary showing score %, rank (or “No rank yet”), and treasure chest state (read from `TreasureChest` for that child). If a completed test is revisited, question view redirects here.
...
- **Next steps**: once PM approves this route/service plan, I’ll scaffold the `diagnostics` app views, `DiagnosticResponse` model + migration, templates, and accompanying functional tests covering the full flow + edge cases (missing answers, revisits, unauthorized parent).

**Update 2025-12-18 — Test Engineer**
...
- Prepared automated test plan (Django `TestCase` + client + fixtures) covering:
  ...
  4. Results view displays score %, rank (or “No rank”), and treasure status; revisiting completed test shows read-only state and disables submission.
  5. Access control scenarios (parent cannot access other child’s test, unauthenticated redirect to login).
...
- Functional tests (`assessments/tests.py`) now cover UI flow (start → answer → submit → results), validation for missing answers, and read-only enforcement; suite green via `./venv/bin/python manage.py test assessments rewards`.
...
**Update 2025-12-18 — Design Owner**
...
- Added shared stylesheet at `core/static/core/design-system.css` exposing the `dsj-*` classes (screen shell, CTA, cards, answer list, alerts, treasure card, chips, wiggle divider). Engineers can attach it to any template via `{% load static %}` → `<link rel="stylesheet" href="{% static 'core/design-system.css' %}">`.
