## 🎟️ Product Ticket: LJ-009 — Deployment & Production Readiness (Heroku)
...
| Child diagnostic flow works              | Start → answer → submit → results |
...
* Clear runbook for local → prod deployment
...
* LJ-002 through LJ-008 complete ✅
...
P0 — Submission readiness / delivery

---

### 🚀 Next Steps by Owner (Source of Truth)

#### ✅ Full Stack Engineer (Primary Owner) — Build + Deploy
...
#### ✅ Test Engineer — Release Gate + Production Smoke
...
**Deliverable:** “Go/No-Go” comment in PR after production smoke.

---

#### ✅ Product Designer — Not Required (Optional Visual Sanity)
...
- Implemented AI “no-crash” behavior for production: `AI_FALLBACK_MODE=stub` on Heroku by default so diagnostics can run even if AI keys aren’t set (configurable via env).
...
**Update 2025-12-17 — Full Stack Engineer (Heroku Deploy Fix)**
- Heroku GitHub build failed because the repo currently tracks `.venv/` (a committed local virtualenv). Heroku’s Python buildpack expects to create its own runtime/venv during compile and can fail when a virtualenv is present in the repo.
...
**Update 2025-12-17 — Full Stack Engineer (Buildpack Fix)**
- Heroku buildpack defaulted to Python 3.14 and failed `collectstatic` with `STATIC_ROOT` missing, indicating the deployment branch didn’t include the latest production settings changes.
...
**Update 2025-12-17 — Full Stack Engineer (Entry Route Fix)**
- Adjusted the root route (`/`) to render a minimal “Welcome” landing page with clear CTAs to `Parent Login` and `Create Parent Account` (instead of showing the design system by default).
- Design system pages remain available at `/design-system/` and `/design-system/elements/` for internal review.
